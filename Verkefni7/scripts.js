...
function formatPrice(price) {
  let price_is = new Intl.NumberFormat("is-IS", {
      style: "currency", 
      currency: "ISK"});
  return price_is.format(price).toString();
}
...
function formatProduct(product, quantity = undefined) {
  ...
  if(quantity === undefined){
    ...
    s += quantity.toString() + "x" + formatPrice(product.price) + " samtals ";
...
function showProducts() {
  for(let i = 0; i < products.length; i++){
    let prod = products[i];
    console.log("#" + prod.id.toString() + " " + prod.title + " — " + 
                      prod.description + " — " + prod.price.toString());
  }
}
...
function addProductToCart() {
  ...
  const idString = prompt('Id vöru:');
  if (!idString) {
    console.error('Id má ekki vera tómt.');
    return;
  }

  const Id = Number.parseInt(idString, 10);

  if(!(validateInteger(Id,1) || Id.isInteger)){
  ...
  const cartQuantString = prompt('Fjöldi af vöru:');
  if (!idString) {
    console.error('Veldu fjölda.');
    return;
  }
...
function checkout() {
  ...
  let totalPrice = 0;
  for(let i = 0; i < cart.lines.length; i++){
    totalPrice += cart.lines[i].quantity*cart.lines[i].product.price;
  }

  console.log("Samtals: " + totalPrice);
